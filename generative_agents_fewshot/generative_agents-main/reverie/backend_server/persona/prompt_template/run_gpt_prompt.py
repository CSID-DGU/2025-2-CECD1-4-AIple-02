...
import datetime
import sys
import random #종합설계1 수정
import csv #종합설계1 수정
import atexit #종합설계1 수정
sys.path.append('../../')
...
#종합설계1 수정
from persona.prompt_template.fewshot_dataset import fewshot_dic
from persona.prompt_template.gpt_structure import *
...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False):
  ...
  prompt_template = "persona/prompt_template/v3_ChatGPT/wake_up_hour_v1.txt"
...
#    return cr
  def __func_clean_up(gpt_response, prompt=""):
    ...
    try:
        ...
        for count, i in enumerate(_cr):
            k = [j.strip() for j in i.split("(duration in minutes:")]
            if len(k) < 2:
                print(f"Invalid format in line: {i}")
                continue
            task = k[0].rstrip(".")
            try:
                duration = int(k[1].split(",")[0].strip())
            except (IndexError, ValueError) as e:
                print(f"Error parsing duration: {e}, line: {i}")
                duration = 0
        ...
        if len(curr_min_slot) > total_expected_min:
            ...
            for i in range(len(curr_min_slot) - total_expected_min):
                curr_min_slot.pop()
        ...
        print(f"Unhandled error in __func_clean_up: {e}")
        return []
  ...
  gpt_param = {"engine": "gpt-3.5-turbo", "max_tokens": 1000,
             ...
             "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/task_decomp_v3.txt"
  ...
  try:
    print(f"DEBUG: fin_output before = {fin_output}")
    print(f"DEBUG: ftime_sum = {ftime_sum}, duration = {duration}")

    if fin_output:
      fin_output[-1][1] += (duration - ftime_sum)
    else:
      print("WARNING: fin_output is empty. Initializing with default task.")
      fin_output.append(["default_task", duration - ftime_sum])

    print(f"DEBUG: fin_output after = {fin_output}")

  except IndexError as e:
    print(f"ERROR: IndexError occurred. fin_output = {fin_output}, ftime_sum = {ftime_sum}, duration = {duration}")
    raise e
...
def run_gpt_prompt_action_arena(action_description,
  ...
  gpt_param = {"engine": "gpt-3.5-turbo", "max_tokens": 15,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/action_location_object_v1.txt"
...
def run_gpt_prompt_action_game_object(action_description,
  ...
  gpt_param = {"engine": "gpt-3.5-turbo", "max_tokens": 15,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/action_object_v2.txt"
...
def run_gpt_prompt_new_decomp_schedule(persona,
  ...
  gpt_param = {"engine": "gpt-3.5-turbo", "max_tokens": 1000,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/new_decomp_schedule_v1.txt"
  prompt_input = create_prompt_input(persona,
                                     main_act_dur,
                                     truncated_act_dur,
                                     start_time_hour,
...
def run_gpt_prompt_decide_to_react(persona, target_persona, retrieved,test_input=None,
  ...
  gpt_param = {"engine": "gpt-3.5-turbo", "max_tokens": 20,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/decide_to_react_v1.txt"
  prompt_input = create_prompt_input(persona, target_persona, retrieved,
                                     test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_create_conversation(persona, target_persona, curr_loc,
  ...
  gpt_param = {"engine": "gpt-3.5-turbo", "max_tokens": 1000,
               ...
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/create_conversation_v2.txt"
...
def run_gpt_prompt_extract_keywords(persona, description, test_input=None, verbose=False):
  ...
  gpt_param = {"engine": "gpt-3.5-turbo", "max_tokens": 50,
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  prompt_template = "persona/prompt_template/v3_ChatGPT/get_keywords_v1.txt"
...
def run_gpt_few_shot(persona, original_text):
    def create_prompt(persona, original_text):
        ...
        prompt += f'MBTI axis: {persona.scratch.get_str_mbti()}\n'   # TODO: mbti 관련 변수 및 함수 구현 필요
        #prompt += f'Innate traits: {persona.scratch.get_str_innate()}\n'
        prompt += f'Learned traits: {persona.scratch.get_str_learned()}\n\n'
        prompt += f'Rephrase input sentence into a natural spoken english of {persona.scratch.get_str_firstname()}.\n' # 20251121수정
        #prompt += f'The output should emphasize MBTI-based behaviors more prominently than innate traits.\n' #20251120추가
        prompt += f'EDITING PERMISSION: You are explicitly allowed to remove or change punctuation marks (ex: exclamation marks ''!'') if they conflict with the target tone.\n' #20191121추가, 문장부호 수정 가능
        prompt += f'Use the speech patterns of other people whose MBTI character is similar to {persona.scratch.get_str_firstname()}.\n\n'
        prompt += (
          "Use the few-shot examples ONLY to understand how this single MBTI axis changes speech style "
          "(for example, emotional vs logical wording, abstract vs concrete focus). "
          "DO NOT copy other personality traits, roles, or life details from the examples.\n\n"
        )
    ...
    def __func_clean_up(gpt_response, prompt=""):
        ...
        persona_name = persona.scratch.get_str_firstname()  # Need to change: persona.scratch.get_str_firstname()
    ...
    def __func_validate(gpt_response, prompt=""):
        try:
        # gpt_response가 raw이든, 이미 정제된 값이든
        # 한 번 더 clean_up을 돌려서 실제 출력 문자열 길이로 판단(즉, clean up이 안된 대답이 흘러 들어올 가능성 차단)
          cleaned = __func_clean_up(gpt_response, prompt="")
          if len(cleaned.strip()) == 0:
              return False
        except Exception as e:
          print("[fewshot][validate_error]", e)
          return False
        return True
    ...
    if persona.scratch.get_str_firstname() not in fewshot_result:
        fewshot_result[persona.scratch.get_str_firstname()] = []
        
    fewshot_result[persona.scratch.get_str_firstname()].append((persona.scratch.get_str_mbti(), original_text, output))
...
#종합설계1 수정
def save_fewshot_data_on_exit():
    """프로그램이 종료될 때 자동으로 CSV 파일을 저장하는 함수"""
    # 현재 시간을 '년월일_시분초' 모양의 문자열로 만듭니다. (예: "20251119_204510")
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    filename = f"fewshot_{timestamp}.csv"
    
    # 데이터가 비어있지 않을 때만 저장
    if not fewshot_result:
        print("\n[알림] 저장할 데이터가 없습니다.")
        return

    print(f"\n[저장 중] 총 {len(fewshot_result)}개의 데이터를 저장하고 있습니다...")
    
    try:
        with open(filename, mode='w', newline='', encoding='utf-8-sig') as file:
            writer = csv.writer(file)
            # 1. 헤더 순서 변경 (MBTI 맨 뒤로)
            writer.writerow(["Character", "Original Text", "Generated Output", "MBTI"])
            
            for name, dialogue_list in fewshot_result.items():
                # 데이터 꺼내는 순서는 저장할 때 넣은 순서(mbti, original, output) 그대로 유지
                for mbti, original, output in dialogue_list:
                    # 2. 실제 저장하는 순서도 헤더에 맞춰서 변경
                    writer.writerow([name, original, output, mbti])
                    
        print(f"[완료] '{filename}' 파일에 성공적으로 저장되었습니다!")
    except Exception as e:
        print(f"[오류] 저장 중 문제가 발생했습니다: {e}")

# 프로그램이 꺼질 때 위 함수가 실행되도록 등록
atexit.register(save_fewshot_data_on_exit)