...
#종합설계1 수정
from persona.prompt_template.run_gpt_prompt import run_gpt_few_shot

#종합설계1 수정
def postprocess_mbti_style(persona, text, n=1):
  try:
        print(f"[DEBUG fewshot hook reached] text='{text[:50]}'")
        if not hasattr(persona, "scratch") or not hasattr(persona.scratch, "get_str_mbti"):
            return text
        axis = persona.scratch.get_str_mbti()
        if not axis:
            return text
        print(f"[fewshot] {persona.scratch.get_str_firstname()} axis={axis} | in='{text[:60]}'")
        styled = run_gpt_few_shot(persona, text)
        print(f"[fewshot] out='{(styled or '')[:60]}'")
        return styled if styled else text
  except Exception as e:
        print("[fewshot][error]", e)
        return text
...
def generate_one_utterance(maze, init_persona, target_persona, retrieved, curr_chat): 
  ...
  utt = postprocess_mbti_style(init_persona, utt, n=1)
...
def generate_next_line(persona, interlocutor_desc, curr_convo, summarized_idea):
  ...
  #종합설계1 수정
  next_line = postprocess_mbti_style(persona, next_line, n=1)
